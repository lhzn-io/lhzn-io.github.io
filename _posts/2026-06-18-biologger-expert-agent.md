...
*   **Sovereign Environment Isolation & Multi-Tier Partitioning:** We structured our deployment via Docker Compose to enforce strict multi-tier privilege isolation, splitting the gateway into distinct "Operator" and "Admin" roles to mirror team collaboration dynamics in the lab. The **Operator Agent** acts as the low-privilege scientific research partner, equipped with specialized skills to run kinematics simulations and manage visualization datasets alongside researchers. Conversely, the **Admin Agent** operates autonomously with high privileges to monitor VRAM allocations, onboard massive raw datasets, and proactively debug or repair the Operator's workspace when execution errors occur. Each role runs its own base agent proxy gateway alongside a shared Selenium/Chromium browser automation node for secure sandboxing.
...
*   **Identity Persona Configuration:** We configured distinct AIEOS metadata (`identity.json`) for each gateway's active workspace. The Operator is prompted as a collaborative kinematics analyst for the WHOI-MPG environment, while the Admin is instructed to act as a trusted, safety-conscious infrastructure maintainer. Both agents are instructed to persist their insights, dataset schema changes, and execution repairs into a shared, git-tracked `learned_facts.md` ledger.
